var emulator = {
  ...
  runFrame: function () {
    if (!emulator.inputQueue.length) {
      console.log('no input in queue')
      emulator.missedInputs += 1
    } else {
      emulator.inputQueue.shift()
    }
  ...
  iNESHandler: function (romFile) {
    ...
    // persistant memory
    if (cartHeader[6] & 0x04) {
      x += 512
      console.log('trainer fie exists but was ignored')
    }
    if (cartHeader[6] & 0x08) {
      ppu.nametableMirroring = 4
      console.log('mirroring mode not yet supported')
    }
